...
internal sealed class XmlSerializationWriterCodeGen : XmlSerializationCodeGen
{
    ...
    internal XmlSerializationWriterCodeGen(IndentedTextWriter writer, TypeScope[] scopes, string access, string className) : base(writer, scopes, access, className)
    {
    }
    ...
    {
        return $"Write{++NextMethodNumber}_{CodeIdentifier2.MakeValidInternal(name)}";
    }
    ...
    private void WriteStructMethod(StructMapping mapping)
    {
        ...
        if (!mapping.TypeDesc.IsAbstract)
        {
            ...
            if (xmlnsMember >= 0)
            {
                ...
                CodeIdentifier2.CheckValidIdentifier(member.Name);
            ...
            for (int i = 0; i < members.Length; i++)
            {
                ...
                if (m.Attribute != null)
                {
                    CodeIdentifier2.CheckValidIdentifier(m.Name);
            ...
            for (int i = 0; i < members.Length; i++)
            {
                ...
                {
                    CodeIdentifier2.CheckValidIdentifier(m.ChoiceIdentifier.MemberName);
    ...
    private static string FindChoiceEnumValue(ElementAccessor element, EnumMapping choiceMapping, bool useReflection)
    {
        ...
        for (int i = 0; i < choiceMapping.Constants!.Length; i++)
        {
            ...
            ReadOnlySpan<char> choiceNs = colon < 0 ? choiceMapping.Namespace.AsSpan() : xmlName.AsSpan(0, colon);
            ReadOnlySpan<char> choiceName = colon < 0 ? xmlName.AsSpan() : xmlName.AsSpan(colon + 1);

            if (choiceName.SequenceEqual(element.Name.AsSpan()))
            {
                if ((element.Form == XmlSchemaForm.Unqualified && choiceNs.IsEmpty) || choiceNs.SequenceEqual(element.Namespace.AsSpan()))
                {
                    if (useReflection)
                        enumValue = choiceMapping.Constants[i].Value.ToString(CultureInfo.InvariantCulture);
                    else
