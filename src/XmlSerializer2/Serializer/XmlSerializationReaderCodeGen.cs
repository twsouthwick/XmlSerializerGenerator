...
internal sealed class XmlSerializationReaderCodeGen : XmlSerializationCodeGen
{
    ...
    internal XmlSerializationReaderCodeGen(IndentedTextWriter writer, TypeScope[] scopes, string access, string className) : base(writer, scopes, access, className)
    {
    }
    ...
    internal void GenerateEnd()
    {
        ...
        Writer.WriteLine();
        foreach (string idName in _idNames.Values.OfType<string>().OrderBy(static k => k))
        {
            Writer.Write("string ");
            Writer.Write(idName);
            Writer.WriteLine(";");
        ...
        Writer.Indent++;
        foreach (var id in _idNames.Keys.OfType<string>().OrderBy(static k => k))
        {
    ...
    private string GetChoiceIdentifierSource(MemberMapping mapping, string parent, TypeDesc parentTypeDesc)
    {
        ...
        CodeIdentifier2.CheckValidIdentifier(mapping.ChoiceIdentifier.MemberName);
    ...
    private string NextMethodName(string name) =>
        $"Read{++NextMethodNumber}_{CodeIdentifier2.MakeValidInternal(name)}";

    private string NextIdName(string name) =>
        $"id{(++_nextIdNumber)}_{CodeIdentifier2.MakeValidInternal(name)}";
    ...
    {
        CodeIdentifier2.CheckValidIdentifier(typeName);
        string? propName = MakeUnique(mapping, $"{typeName}Values");
        if (propName == null) return CodeIdentifier2.GetCSharpName(typeName);
        string? memberName = MakeUnique(mapping, $"_{propName}");
        propName = CodeIdentifier2.GetCSharpName(propName);
        ...
        for (int i = 0; i < constants.Length; i++)
        {
            ...
            if (!mapping.TypeDesc!.UseReflection)
            {
                ...
                Writer.Write(mapping.TypeDesc.CSharpName);
                Writer.Write(".@");
                CodeIdentifier2.CheckValidIdentifier(constants[i].Name);
                Writer.Write(constants[i].Name);
            }
            else
            {
                Writer.Write(", ");
                Writer.Write($"{constants[i].Value}L");
            }
    ...
    private void WriteLiteralStructMethod(StructMapping structMapping)
    {
        ...
        else
        {
            ...
            {
                MemberMapping mapping = mappings[i];
                CodeIdentifier2.CheckValidIdentifier(mapping.Name);
    ...
    private void WriteAddCollectionFixup(TypeDesc typeDesc, bool readOnly, string memberSource, string targetSource)
    {
        ...
        if (create == null)
        {
            string createName = $"create{++_nextCreateMethodNumber}_{typeDesc.Name}";
    ...
    private static string ExpectedElements(Member[] members)
    {
        ...
        ReflectionAwareCodeGen.WriteQuotedCSharpString(new IndentedTextWriter(writer), qnames);
    ...
    private void WriteElement(string source, string? arrayName, string? choiceSource, ElementAccessor element, ChoiceIdentifierAccessor? choice, string? checkSpecified, bool checkForNull, bool readOnly, int fixupIndex, int elementIndex)
    {
        ...
        if (choice != null)
        {
            ...
            Writer.Write(" = ");
            CodeIdentifier2.CheckValidIdentifier(choice.MemberIds![elementIndex]);
