...
public partial class Program
{
    ...
    {
        var srRegex = SR_Regex();

        var dir = Path.Combine(GetGitDir(), "src");
        var srFile = Path.Combine(dir, "XmlSerializer2", "SR.cs");
        ...
        indented.WriteLine();
        indented.WriteLine("public static string Format(string name, params object?[] args) => string.Format(System.Globalization.CultureInfo.InvariantCulture, name, args);");
        indented.WriteLine();

        foreach (var item in sr.OrderBy(s => s))
        {
            if (item is "GetString" or "Format")
            {
                continue;
            }
    ...
    [GeneratedRegex(@"SR\.\w*", RegexOptions.Compiled)]
    private static partial Regex SR_Regex();
}
